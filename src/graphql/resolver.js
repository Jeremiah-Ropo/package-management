...
const user = async (req) => await User.findById(req.user.id);
...
const createPackage = async (
  ...
  if (!req.user)
    return new CustomError("Kindly login with your credentials", 401);
...
const deletePackage = async ({ id }, req) => {
  if (!req.user)
    throw new CustomError("Kindly login with your credentials", 401);
